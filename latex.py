...
def generate_latex_tables(df):
    """
    Generates LaTeX tables from the benchmark data and prints them to the console.

    Args:
        df (pandas.DataFrame): The DataFrame with benchmark data.
    """
    if df is None or df.empty:
        print("Cannot generate LaTeX tables because no data was provided.")
        return
    ...
    for size in table_sizes:
        ...
        print(f"\\begin{{table}}[H] % Table for size {size:,}")
        print(f"    \\centering")
        print(f"    \\caption{{Performance Results for Table Size: {size:,}}}")
        print(f"    \\label{{tab:results_{size}}}")
        print(f"    \\begin{{tabular}}{{|c|c|c|c|}}")
        print(f"        \\hline")
        
        # Header row
        print(f"        \\textbf{{Uniqueness}} & "
              f"\\textbf{{Hash-Join Time (s)}} & "
              f"\\textbf{{Group-Join Time (s)}} & "
              f"\\textbf{{Speedup Factor}} \\\\ \\hline")

        # Data rows
        for index, row in subset.iterrows():
            print(f"        {row['uniqueness']*100:.0f}\\% & "
                  f"{row['time_hashjoin_s']:.6f} & "
                  f"{row['time_groupjoin_s']:.6f} & "
                  f"\\textbf{{{row['speedup']:.2f}x}} \\\\ \\hline")
...
if __name__ == "__main__":
    ...
    generate_latex_tables(benchmark_df)
    print("\nScript finished.")
